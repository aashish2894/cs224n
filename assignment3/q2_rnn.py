...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence)>=max_length:
            sent1 = sentence[0:max_length]
            lab1 = labels[0:max_length]
            mask1 = [True]*max_length
        elif len(sentence)<max_length:
            sent1 = sentence + [zero_vector]*(max_length-len(sentence))
            lab1 = labels + [zero_label]*(max_length-len(labels))
            mask1 = [True]*len(sentence) + [False]*(max_length-len(labels))
        ret.append((sent1,lab1,mask1))
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=preds)
