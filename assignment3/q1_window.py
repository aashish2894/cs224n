    	...
    	for i in range(len(labels)):
    		...
    		for k in range(1,window_size+1):
    			if j+k > len(labels)-1 :
    				...
    				list1.append(sentence[j+k])
    		list1 = flatten(list1)
    		windowed_data.append((list1,labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name="W1", shape=[self.config.n_window_features*self.config.embed_size, self.config.hidden_size], initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(name="b1", shape=(self.config.hidden_size,), initializer=tf.zeros_initializer())
        a1 = tf.add(tf.matmul(x,W),b1)
        h1 = tf.nn.relu(a1)
        h1_drop = tf.nn.dropout(h1,dropout_rate)
        U = tf.get_variable(name="U1", shape=[self.config.hidden_size, self.config.n_classes], initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable(name="b2", shape=(self.config.n_classes,), initializer=tf.zeros_initializer())
        pred = tf.add(tf.matmul(h1_drop,U),b2)
